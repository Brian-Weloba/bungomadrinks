...
function Details() {
  ...
  const fetchDetails = () => {
    axios.get(prodOptionsUrl).then((res) => {
    ...
    axios.get(url).then((response) => {
      ...
      const defOpt = defaultOptionObject.find(
        (option) => option.productOptionId === prod.defaultOption
      );
  ...
  function handleChange(e) {
    ...
    let optionPrice = productOptions.find(
      (option) => option.productOptionId === Number(e.target.value)
    ).productOptionPrice;
      ...
      >
        ...
        >
          ...
          >
            ...
            <Typography component={"div"} variant={"h5"}>
              KES {price.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")}{" "}
            </Typography>
            <FormControl>
